...
- **队列满会丢日志行**（不阻塞转发，也不影响原文归档）。22,000 RPS 下实测丢弃率 **4.0%**
  （摘掉原文写入前是 26%）。丢弃数、队列占用、上批耗时全部显示在概览页
  「异步落库管道」卡片里 —— **隐性丢数据是不可接受的，所以必须可见**。
  队列容量由 `GATEWAY_LOG_QUEUE_SIZE` 控制，因为每条只占 ~350 字节，
  要降低丢弃率可以放心往大调（10 万条也只有 ~35MB）。
